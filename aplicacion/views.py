...
def form_docente(request):
    if request.method == 'POST':
        ...
        apellido_docente = request.POST.get('apellido-docente')
        cedula_docente = request.POST.get('cedula-docente')
        tel_ofi_docente = request.POST.get('tel-ofi-docente')
        tel_per_docente = request.POST.get('tel-per-docente')
        correo_inst_docente = request.POST.get('correo-inst-docente')
        porcentaje_planes_completados = request.POST.get('porcentaje-planes-completados')

        rango_estudiantes = request.POST.get('rango-estudiantes')
        ...
        tiempo_promedio_ingles = request.POST.get('tiempo_promedio_ingles')

        senalizaciones_centro_ingles = request.POST.get('senalizaciones-centro-ingles') == 'si'
        cantidad_senalizaciones_centro = request.POST.get('cantidad-senalizaciones-centro')
        tipo_senaletica = request.POST.getlist('tipo-senaletica')  # getlist para checkboxes
        senaletica_aula = request.POST.get('senaletica')
        cantidad_senalizaciones_aula = request.POST.get('cantidad-senalizaciones-aula')
        porcentaje_tiempo = request.POST.get('porcentaje_tiempo', 0)  # Default a 0 si está vacío
        ...
        teatro = request.POST.get('teatro')
        coreografia = request.POST.get('coreografia')
        ...
        nivel_actual = request.POST.getlist('nivel-actual')
        titulo_ensenanza = request.POST.get('titulo-ensenanza')
        nivel_titulo = request.POST.get('nivel-titulo-select', None)
        certificacion_ingles = request.POST.get('certificacion-ingles')
        nivel_ingles_docente = request.POST.get('nivel-ingles-docente')
        nivel_ingles_docente_no_certificado = request.POST.get('nivel-ingles-docente-no-certificado')
        renovar_certificacion = request.POST.get('renovar-certificacion')
        frecuencia_uso_recursos = request.POST.get('frecuencia-recursos')
        ...
        encargado_ppb = request.POST.get('encargado-ppb', '')
        
        cursos_educacion_continua = {
            'curso': {
                'ano': request.POST.get('curso-ano'),
                'horas': request.POST.get('curso-horas')
            },
            'taller': {
                'ano': request.POST.get('taller-ano'),
                'horas': request.POST.get('taller-horas')
            },
            'seminario': {
                'ano': request.POST.get('seminario-ano'),
                'horas': request.POST.get('seminario-horas')
            },
            'diplomado': {
                'ano': request.POST.get('diplomado-ano'),
                'horas': request.POST.get('diplomado-horas')
            },
            'entrenamiento': {
                'ano': request.POST.get('entrenamiento-ano'),
                'horas': request.POST.get('entrenamiento-horas')
            },
            'otros': {
                'ano': request.POST.get('otros-ano'),
                'horas': request.POST.get('otros-horas')
            }
        }

        licenciatura_ano = request.POST.get('licenciatura-ano') or None
        postgrado_ano = request.POST.get('postgrado-ano') or None
        maestria_ano = request.POST.get('maestria-ano') or None
        doctorado_ano = request.POST.get('doctorado-ano') or None


        docente = Docente(
                licenciatura_ano=licenciatura_ano, postgrado_ano=postgrado_ano, maestria_ano=maestria_ano, doctorado_ano=doctorado_ano,

                nombre=nombre_docente,
                apellido=apellido_docente,
                cedula=cedula_docente,
                telefono_oficina=tel_ofi_docente,
                telefono_personal=tel_per_docente,
                correo_institucional=correo_inst_docente,
                porcentaje_planes_completados=porcentaje_planes_completados,
                rango_estudiantes=rango_estudiantes,
                frecuencia_habla_ingles=frecuencia_habla_ingles,
                porcentaje_tiempo_ingles=porcentaje_tiempo_ingles,
                incentiva_hablar_ingles=incentiva_hablar_ingles,
                porcentaje_promueve_ingles=porcentaje_promueve_ingles,
                tiempo_dialogo_ingles=tiempo_dialogo_ingles,
                tiempo_promedio_ingles=tiempo_promedio_ingles,
                senalizaciones_centro_ingles=senalizaciones_centro_ingles,
                cantidad_senalizaciones_centro=cantidad_senalizaciones_centro,
                tipo_senaletica=tipo_senaletica,  # Asegúrate de manejar correctamente los campos de tipo lista
                senaletica_aula=senaletica_aula,
                cantidad_senalizaciones_aula=cantidad_senalizaciones_aula,
                evidence_file=evidence_file,
                frecuencia_interaccion_directivos=frecuencia_interaccion_directivos,
                frecuencia_interaccion_docente=frecuencia_interaccion_docente,
                frecuencia_interaccion_padres=frecuencia_interaccion_padres,
                interactua_estudiantes=interactua_estudiantes,
                musica=musica,
                teatro=teatro,
                ...
                titulo_ensenanza=titulo_ensenanza,
                nivel_titulo=nivel_titulo,
                certificacion_ingles=certificacion_ingles,
                nivel_ingles_docente=nivel_ingles_docente,
                nivel_ingles_docente_no_certificado=nivel_ingles_docente_no_certificado,
                renovar_certificacion=renovar_certificacion,
                cursos_educacion_continua=cursos_educacion_continua,
                frecuencia_uso_recursos=frecuencia_uso_recursos,
                acceso_recursos=acceso_recursos,
                importancia_formacion_bilingue=importancia_formacion_bilingue,
                valoracion_formacion_bilingue=valoracion_formacion_bilingue,
                prioridad_ppb=prioridad_ppb,
                participacion_padres=participacion_padres,
                cooperacion_ppb=cooperacion_ppb,
                encargado_ppb=encargado_ppb
            )
        docente.save()
